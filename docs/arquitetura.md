...
permanecem na rede interna e não publicam portas HTTP no host (ver [ADR-003](adr/ADR-003-autorizacao-administrativa.md)).
...
Para leitura rápida do painel, `GET /partidas/catalog` oferece catálogo ordenado, paginado e cacheado em Redis (ver [ADR-006](adr/ADR-006-estrategia-de-cache.md)) nas views `OPEN`, `LIVE`, `FINISHED` e `CANCELED`. O campo
...
PostgreSQL, usa Redis para cache de saldo (ver [ADR-006](adr/ADR-006-estrategia-de-cache.md)) e consome/publica eventos pelo Kafka.
...
## Resiliência e Dead Letter Queue

O `carteira-service` implementa resiliência em múltiplas camadas para lidar com
falhas transitórias e permanentes:

### Consumidores Kafka

Os consumidores de `bet-events` e `user-events` utilizam:
- **`@Retry` com backoff**: reprocessamento automático com delay de 2 segundos
  entre tentativas (até 3 tentativas para bet-events).
- **`@CircuitBreaker`**: abre o circuito após 4 falhas consecutivas,
  impedindo cascata de erros por 5 segundos.
- **DLQ (Dead Letter Queue)**: mensagens que esgotam os retries são enviadas
  automaticamente para `bet-events-dlq` e `user-events-dlq` respectivamente
  (`failure-strategy=dead-letter-queue`). Isso garante que mensagens não
  processáveis não bloqueiem o consumo das demais.

### Cache Redis

Todas as operações de leitura, escrita e invalidação no Redis
(`RedisWalletCache`) são protegidas por:
- **`@Retry(maxRetries = 2, delay = 200)`**: retry rápido para falhas
  transitórias de rede.
- **`@CircuitBreaker(requestVolumeThreshold = 4, delay = 5000)`**: evita
  saturar um Redis que esteja degradado.
- **`@Fallback`**: degradação graciosa — se o Redis estiver indisponível, as
  leituras retornam `Optional.empty()` e o serviço recorre ao PostgreSQL.

### Depósitos PIX

O fluxo de depósito usa reconciliação idempotente: um timeout na chamada ao
provedor não apaga a solicitação; `POST /wallet/me/deposits/{id}/reconcile`
retoma a cobrança de forma segura. Webhook e reconciliação convergem no mesmo
caso de uso transacional. Ver [ADR-005](adr/ADR-005-provedor-pagamento-sandbox.md).
